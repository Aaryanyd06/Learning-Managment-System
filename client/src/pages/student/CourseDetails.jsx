...
const CourseDetails = () => {
  ...
  const {allCourses, calculateRating, calculateCourseDuration, calculateNoOfLectures, calulcateChapterTime, currency} = useContext(AppContext)
  ...
  return courseData ? (
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      ...
      <div className='absolute inset-0 left-0 w-full h-section-height -z-1 bg-gradient-to-b from-cyan-100/70'></div>

      <div className='max-w-xl z-10 text-gray-500'><h1 className='md:text-course-details-heading-large text-course-details-heading-small font-semibold text-gray-800'>{courseData.courseTitle}</h1>
      ...
      <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
        ...
        <p>Course by <span className='text-blue-600 underline'>Aaryan</span></p>
            ...
            {courseData.courseContent.map((chapter, index) =>(
              <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={()=>{
                  ...
                  <p className='text-sm md:text-default'>{chapter.chapterContent.length} lectures - {calulcateChapterTime(chapter)}</p>
                </div>
                  ...
                  '>
                    {chapter.chapterContent.map((lecture, index)=>(
                      <li key={index} className='flex items-start gap-2 py-1'>
                        <img src={assets.play_icon} alt="play-ucon" className='w-4 h-4 mt-1 ' />
